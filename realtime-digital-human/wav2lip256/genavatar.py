...
def video2imgs(vid_path, save_path, cut_frame=10000000):
    ...
    with tqdm(total=min(total_frames, cut_frame),
        ...
        while count < cut_frame:
            ...
            if ret:
                ...
                # alpha = matte(frame) / 255.0
                # composite_image = (frame * alpha[:, :, np.newaxis] + bg *
                #                    (1 - alpha[:, :, np.newaxis])).astype(
                #                        np.uint8)
                # ---- 需要替换成其他背景的时候使用 ----
                composite_image = frame.copy()
                cv2.imwrite(f"{save_path}/{count:08d}.png", composite_image)
...
if __name__ == "__main__":
    ...
    avatar_path = os.path.join(project_root_path,
                               f"data/avatars/{args.avatar_id}")
    ...
    video2imgs(args.video_path, full_imgs_path)
